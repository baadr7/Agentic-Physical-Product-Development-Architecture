...
export async function GET(req: NextRequest, { params }: { params: { run_id: string } }) {
  const proxied = await tryProxy(req, `/api/v1/runs/${params.run_id}`, { method: 'GET' });
  if (proxied) return proxied;
  return Response.json({ detail: 'FastAPI unavailable' }, { status: 503 });
}

export async function PATCH(req: NextRequest, { params }: { params: { run_id: string } }) {
  const raw = await req.text();
  const proxied = await tryProxy(req, `/api/v1/runs/${params.run_id}`, {
    method: 'PATCH',
